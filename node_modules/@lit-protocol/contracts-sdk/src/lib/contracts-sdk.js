    ...
    constructor(args) {
        ...
        this.connect = async () => {
            ...
            // ----------------------------------------------
                ...
                this.provider = new ethers_1.ethers.providers.StaticJsonRpcProvider(this.rpc);
            }
            // ======================================
            //          CUSTOM PRIVATE KEY
            // ======================================
...
https://developer.litprotocol.com/v3/sdk/wallets/auth-methods/#auth-method-scopes
    ...
    static async getStakingContract(network, context, rpcUrl) {
        let provider;
        rpcUrl = constants_1.RPC_URL_BY_NETWORK[network];
        if (context && 'provider' in context) {
            provider = context.provider;
        }
        else {
            provider = new ethers_1.ethers.providers.StaticJsonRpcProvider(rpcUrl);
        }
        if (!context) {
    ...
    // context?: LitContractContext | LitContractResolverContext
        ...
        if (!constants_1.NETWORK_CONTEXT_BY_NETWORK[network]) {
            throw new Error(`[_resolveContractContext] Unsupported network: ${network}`);
        }
...
LitContracts.getValidators = async (network, context, rpcUrl, nodeProtocol) => {
    ...
    const networks = activeValidatorStructs.map((item) => {
        ...
        if (centralisation === 'centralised') {
            ...
            if (network === constants_1.LIT_NETWORK.Cayenne &&
                !port.toString().startsWith('8')) {
                throw new Error(`Invalid port: ${port} for the ${centralisation} ${network} network. Expected range: 8470 - 8479`);
            }
        }
...
LitContracts.getConnectionInfo = async ({ litNetwork, networkContext, rpcUrl, nodeProtocol, }) => {
    const stakingContract = await LitContracts.getStakingContract(litNetwork, networkContext, rpcUrl);
    ...
    const networks = activeValidatorStructs.map((item) => {
        ...
        if (centralisation === 'centralised') {
            // Validate if it's cayenne AND port range is 8470 - 8479, if not, throw error
            if (litNetwork === constants_1.LIT_NETWORK.Cayenne &&
                !(port >= 8470 && port <= 8479)) {
                throw new Error(`Invalid port: ${port} for the ${centralisation} ${litNetwork} network. Expected range: 8470 - 8479`);
            }
        }
        const url = `${protocol}${ip}:${port}`;
        LitContracts.logger.debug("Validator's URL:", url);
