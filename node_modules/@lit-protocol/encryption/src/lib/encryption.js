...
async function decryptFromJson(params) {
    ...
    if (parsedJsonData.dataType === 'string') {
        ...
        throw new Error(`dataType of ${parsedJsonData.dataType} is not valid. Must be 'string' or 'file'.`);
    }
}
...
const zipAndEncryptString = async (params, litNodeClient) => {
    ...
    return (0, exports.encryptZip)({ zip, ...params }, litNodeClient);
};
...
*/
const zipAndEncryptFiles = async (files, params, litNodeClient) => {
    // let's zip em
    let zip;
    try {
        zip = new JSZip.default();
...
const encryptFileAndZipWithMetadata = async (params) => {
    const { sessionSigs, accessControlConditions, evmContractConditions, solRpcConditions, unifiedAccessControlConditions, chain, file, litNodeClient, readme, } = params;
    // -- validate
    const paramsIsSafe = (0, params_validators_1.safeParams)({
